...
const products = (props) => {
	...
	if (category === 'Price Low To High') {
		loadedProducts = (
			<Row>
				{propProduct.sort((a, b) => a.price - b.price).map((product) => (
					<Col xs={6} sm={6} md={4} lg={4} key={product.id}>
						<Product product={product} />
					</Col>
				))}
	...
	if (category === 'Price High To Low') {
		loadedProducts = (
			<Row>
				{propProduct.sort((a, b) => b.price - a.price).map((product) => (
					<Col xs={6} sm={6} md={4} lg={4} key={product.id}>
						<Product product={product} />
					</Col>
				))}
	...
	if (category === 'Just In') {
		loadedProducts = (
			<Row>
				{propProduct.filter((prod) => prod.productMsgInfo === 'Just In').map((product) => (
					<Col xs={6} sm={6} md={4} lg={4} key={product.id}>
						<Product product={product} />
					</Col>
				))}
	...
	return (
		<Container className="products">
			...
			<Row>
				<Col xs={12} sm={6} md={6}>
					<h4 style={{ fontWeight: '300' }} className="page-title">
						Products
					</h4>
					<small className="text-muted">{'(' + props.products.length + ' items)'}</small>
				</Col>
				<Col xs={12} sm={6} md={6}>
					<Form.Group controlId="exampleForm.ControlSelect1" style={{ float: 'right' }}>
						<Form.Control
							as="select"
							value={category}
							onChange={(event) => {
